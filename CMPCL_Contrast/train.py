...
from utils.prototype_aug_dist_estimator import prototype_aug_dist_estimator
from utils.prototype_pixel_aug_dist_estimator import prototype_pixel_aug_dist_estimator
...
def main():
    ...
    for i in range(proto_aug_num):
        proto_transform_feat_estimator = prototype_aug_dist_estimator(feature_num=1024)
        proto_aug_lists.append(proto_transform_feat_estimator)

    proto_pixel_aug_lists = []
    for i in range(proto_aug_num):
        proto_transform_pixel_estimator = prototype_pixel_aug_dist_estimator(feature_num=1024)
        proto_pixel_aug_lists.append(proto_transform_pixel_estimator)
    ...
    while i < args.max_iter:
        ...
        i = train(train_loader, covstat_val_loaders, criterion_val, net, optim, epoch, writer, scheduler, args.max_iter, proto_feat_estimator,
                  proto_aug_lists, proto_pixel_aug_lists, proto_aug_num, proto_uncertainty_estimator, pcl_criterion, hard_pcl_criterion)
    ...
    print('save proto')
    save_source_proto_numpy(proto_feat_estimator.Proto.detach())
    print('save proto aug')
    for i in range(proto_aug_num):
        save_transform_proto_numpy(proto_pixel_aug_lists[i].Proto.detach(), dataset='transformed_pixel_source_'+ str(i))
    for i in range(proto_aug_num):
        save_transform_proto_numpy(proto_aug_lists[i].Proto.detach(), dataset='transformed_class_source_'+ str(i))
    print('save proto uncertainty')
    save_proto_uncertainty_numpy(proto_uncertainty_estimator.Proto_uncentainty.detach())
...
def train(train_loader, covstat_val_loaders, criterion_val, net, optim, curr_epoch, writer, scheduler, max_iter,
          proto_feat_estimator=None, proto_aug_lists=None, proto_pixel_aug_lists=None, proto_aug_num=None, proto_uncertainty_estimator=None,
    ...
    for i, data in enumerate(train_loader):
        if curr_iter >= max_iter:
            save_source_proto_numpy(proto_feat_estimator.Proto.detach(), curr_iter)
            break
        ...
        for di, ingredients in enumerate(zip(inputs, gts, aux_gts)):
            ...
            for n in range(proto_aug_num):
                proto_pixel_aug_lists[n].update(x=proto_feat_estimator.Proto.detach())
                proto_uncertainty = torch.abs(
                    proto_pixel_aug_lists[n].Proto.detach() - proto_feat_estimator.Proto.detach()).cuda(
                    non_blocking=True)
            ...
            # print(proto_uncertainty_pro.shape)
            ...
            loss_proto_feat = pcl_criterion(proto_aug_num, Proto=weighted_proto_feats_list, feat=feats, labels=src_mask)

            loss_hard_proto_feat = hard_pcl_criterion(proto_aug_num, Proto1=proto_aug_lists,
            ...
            outputs_index += 1
            total_loss = main_loss + (0.4 * aux_loss) + 0.1 * loss_proto_feat + 0.1 * loss_hard_proto_feat
...
def validate_for_transform_img(val_loader, dataset, net, criterion, optim, scheduler, curr_epoch, writer, curr_iter,
                               save_pth=False, proto_transform_feat_estimator=None):
    ...
    for val_idx, data in enumerate(val_loader):
        ...
        if val_idx >= 499:
            break
    if save_pth:
        save_transform_proto_numpy(proto_transform_feat_estimator.Proto.detach(), dataset=dataset)

    # return proto_transform_feat_estimator
    return
...
def save_source_proto_numpy(feature_maps, iteration=40000, dataset='source'):
    file_fullpath = './visualization/source_proto/'
    file_name = str(args.date) + '_' + str(args.exp) + '_' + str(iteration) + '_' + str(dataset)
    feature_map = feature_maps.data.cpu().numpy()   # H X D
    np.save(file_fullpath + file_name, feature_map)


def save_transform_proto_numpy(feature_maps, iteration=40000, dataset='transformed_source'):
    file_fullpath = './visualization/transform_proto/'
    file_name = str(args.date) + '_' + str(args.exp) + '_' + str(iteration) + '_' + str(dataset)
    feature_map = feature_maps.data.cpu().numpy()  # H X D
    np.save(file_fullpath + file_name, feature_map)


def save_proto_uncertainty_numpy(feature_maps, iteration=40000, dataset='proto_uncertainty'):
    file_fullpath = './visualization/proto_uncertainty/'
    file_name = str(args.date) + '_' + str(args.exp) + '_' + str(iteration) + '_' + str(dataset)
    feature_map = feature_maps.data.cpu().numpy()  # H X D
    np.save(file_fullpath + file_name, feature_map)
